      ...
      if(props.style) {
        ...
        for (var arg in styles) {
          if (styles.hasOwnProperty(arg)) {
            ...
            else if (arg === "text-align" && ["right", "left", "center"].includes(val)) {
              props.formatting_options = { align : val };
            }
            else if (arg === "text-indent") {
              const matches = val.match(/(\d+)%/);
              if (matches) props.formatting_options = { indent : 72 * parseInt(matches[1]) };
            }
          }
        }
