...
class html2xml {
  ...
  toJSON() {
    ...
    try {
      // XXX commented next 3 lines as it has been taken care of by sanitisation process in the formatter
  ...
  buildText(child) {
    ...
    if(props) {
      ...
      if(props.list) {
        ...
        const list = `
          <w:pPr>
            <w:pStyle w:val="${'Normal' || props.listTypeXML}"/>
            <w:numPr>
      ...
      if(props.style) {
        ...
        for (var arg in styles) {
          if (styles.hasOwnProperty(arg)) {
            ...
            if (arg === "text-align" && ["right", "left", "center"].includes(val)) {
              props.formatting_options = { ...props.formatting_options, align : val };
            }
            if (arg === "text-indent") {
              const matches = val.match(/(\d+)%/);
              if (matches) props.formatting_options = { ...props.formatting_options, indent : 72 * parseInt(matches[1]) };
            }
          }
        }
      ...
      if (props.formatting_options) {
        ...
        fopts.push(`<w:pPr>`);
        fopts.push(
          `<w:pStyle w:val="${props.formatting_options.style || "Normal"}"/>`
        );
  ...
  convert(item) {
    const tableTags = ["table", "tbody", "tr", "td", "thead", "th", "tfoot", "p"]
