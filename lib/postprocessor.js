...
const postprocessor = {
  process: function (template, data, options, callback) {
    var fileType = template.extension;
    const filestore = new FileStore();
    switch (fileType) {
      case "odt":
        new OdtPostProcessor(template, data, options, filestore, callback);
        return;
      case "docx":
        new DocxPostProcessor(template, data, options, filestore, callback);
        return;
      case "xlsx":
      case "ods":
      default:
...
function fetchImages(urls) {
  ...
  const fetches = [...new Set(urls || [])].map((url) =>
    axios({
      method: "get",
      url,
...
class OdtPostProcessor {
  constructor(template, data, options, filestore, callback) {
    ...
    fetchImages(frameUrls).then((resolved) => {
      filestore.imagesFetched(resolved.map((r) => r.value));
      // Use base64 data to create new file and update references
      contentXml.data = contentXml.data.replaceAll(
        allframes,
        function (drawFrame) {
          const { mime, content, error } = filestore.getImage(
      ...
      return callback(
        filestore.fetchErrorMessage ? new Error("Image fetching error") : undefined
      );
    }).catch(e => {
